...
class SafeEyes(Gtk.Application):
    ...
    def do_command_line(self, command_line):
        ...
        if cli.get("about"):
            ...
            if not self.take_break(requested_break_type):
                # The core wasn't ready to honour the break (no scheduler wait
                # armed yet, e.g. just after the primary launched). Signal the
                # invoking process to retry rather than report success:
                # ChickenBlocker's `safeeyes -t/-b` driver retries on non-zero
                # exit, and crucially this keeps the primary ALIVE (vs. the old
                # uncaught raise that crashed the whole process, generating an
                # apport report and forcing a relaunch).
    ...
    def take_break(self, break_type: typing.Optional[BreakType] = None) -> bool:
        """Take a break now. Returns True if honoured (or nothing to do),
        False if the core could not honour it right now (no scheduler wait
        armed yet) so the remote CLI caller can retry."""
        return self.safe_eyes_core.take_break(break_type)
