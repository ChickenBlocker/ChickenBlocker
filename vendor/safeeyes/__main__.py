...
def main() -> None:
    ...
    # Propagate the run() exit code to the process: for a *remote* invocation
    # (e.g. ChickenBlocker's `safeeyes -t/-b`) this is the value returned by
    # do_command_line, which is non-zero when the break couldn't be honoured so
    # the daemon's retry loop retries instead of crashing the primary.
    sys.exit(safe_eyes.run(sys.argv))
