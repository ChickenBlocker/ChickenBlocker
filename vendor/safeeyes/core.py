...
class SafeEyesCore:
    ...
    def take_break(self, break_type: typing.Optional[BreakType] = None) -> bool:
        """Calling this method stops the scheduler and show the next break
        screen.

        Returns True if the break was queued, or if the core is not in a state
        where a break can be started (nothing to do). Returns False if the
        scheduler could not honour the request right now because no wait is
        armed (e.g. the primary just launched and the first __wait_for has not
        been scheduled yet). Returning False lets the remote CLI caller
        (ChickenBlocker's `safeeyes -t/-b`) exit non-zero and retry shortly,
        instead of the uncaught raise from __wakeup_scheduler killing the whole
        primary process.
        """
        if self._break_queue is None:
            return True
        ...
        # No wait is armed and we are not mid-hook: the scheduler is idle (the
        # core just started and __scheduler_job hasn't reached __wait_for, or it
        # is transiently between cycles). __wakeup_scheduler would raise
        # "trying to queue action while core is not running" here, which
        # propagates out of do_command_line and crashes the primary. Defer to
        # the caller instead.
        if self._callback is None and not self._firing_hook:
            logging.warning(
                "take_break requested but no scheduler wait is armed yet; "
                "caller should retry"
            )
            return False
